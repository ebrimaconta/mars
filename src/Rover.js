  ...
  move(move) {
    ...
    if (move === 'F' || move === 'B') {
      ...
      const pos = currentState.position.map(function (p, idx) {
        return calculateMove(p + roverTurnOptions[currentState.direction][idx] * moveDir[idx], props.dimensions[idx]);
      });
      ...
      let newDirection = calculateMove(currentState.direction + (move === 'R' ? 1 : -1), 4);
      this.setState({
        history: history.concat([
          {
            move: move,
  ...
  render() {
    ...
    return (
      <div className='container'>
        <div className='board'>
          <Board dimensions={this.props.dimensions} position={current.position} direction={current.direction} />
        </div>
        <div className='controls'>
          ...
          <Button action='RUN' disabled={!this.checkForMove()} onClick={() => this.executeNextMove()} />
        </div>
      </div>
    );
  }
